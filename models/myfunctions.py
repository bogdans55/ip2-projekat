...
def write_results(filename, classes):
    ...
    results = pd.concat([results ,test(data, RandomForestClassifier, rf, rf_params, 'rf', classes)], ignore_index=True)
    results = pd.concat([results ,test(data, RandomForestClassifier, rf_over, rf_params, 'rf_over', classes)], ignore_index=True)
    ...
    results = pd.concat([results ,test(data, LogisticRegression, log_reg, log_reg_params, 'log_reg', classes)], ignore_index=True)
    results = pd.concat([results ,test(data, LogisticRegression, log_reg_cw, log_reg_cw_params, 'log_reg_cw', classes)], ignore_index=True)
    results = pd.concat([results ,test(data, LogisticRegression, log_reg_over, log_reg_params, 'log_reg_over', classes)], ignore_index=True)
    ...
    results = pd.concat([results ,test(data, MLPClassifier, nn, nn_params, 'nn', classes)], ignore_index=True)
    results = pd.concat([results ,test(data, MLPClassifier, nn_over, nn_params, 'nn_over', classes)], ignore_index=True)
    ...
    results = pd.concat([results ,test(data, XGBClassifier, xg, xg_params, 'xg')], ignore_index=True)
    results = pd.concat([results ,test(data, XGBClassifier, xg_over, xg_params, 'xg_over')], ignore_index=True)
